...
const Header = ({ handleScrollToTop }) => {
  ...
  return (
    <header className="font-semibold border-b bg-[#DEF2F1] border-gray-300 md:border-b md:border-gray-200">
      <nav className="flex flex-row items-center p-4 px-2  h-max md:flex-col md:items-center xl:flex-row">
        ...
        <div>
          <ul
            className={`pt-5 md:pt-0 bg-[#E2F0F9] w-1/2 h-screen flex transition-transform duration-300 transform md:transform-none md:transition-none flex-col absolute top-0 right-0 text-center border border-gray-200 md:border-none md:static md:h-auto md:w-auto md:flex md:flex-row md:text-base
          ...
          >
            {urls.map((element, index) => (
              <Link
                to={element.url}
                key={index}
                className={`md:text-center px-4 py-4 sm:py-2 transition duration-100 ease-out hover:bg-gray-400 hover:text-white md:hover:scale-105 cursor-pointer 
                ${currentUrl === element.url ? "bg-gray-800 text-white" : ""}`}
                onClick={() => handleMenuToggle()}
              >
                {element.label}
              </Link>
